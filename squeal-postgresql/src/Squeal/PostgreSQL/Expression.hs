...
field
  :: Has field row ty
  => SchemumExpression schema_alias tydef db ('Typedef ('PGcomposite row)) -- ^ row type
  -> Alias field -- ^ field name
  -> Expression db from grouping params ('NotNull ('PGcomposite row))
  -> Expression db from grouping params ty
field td fld expr = UnsafeExpression $
  parenthesized (renderExpression expr <> "::" <> renderSchemumExpression td)
    <> "." <> renderAlias fld
...
typedef
  :: SchemumExpression schema_alias ty_alias db ('Typedef ty)
  -- ^ user defined type
  -> TypeExpression db (nullity ty)
typedef = UnsafeTypeExpression . renderSchemumExpression

-- | The composite type corresponding to a `Table` definition can be expressed
-- by its alias.
typetable
  :: SchemumExpression schema_alias table_alias db ('Table table)
  -- ^ user defined table
  -> TypeExpression db (nullity ('PGcomposite (TableToRow table)))
typetable = UnsafeTypeExpression . renderSchemumExpression

-- | The composite type corresponding to a `View` definition can be expressed
-- by its alias.
typeview
  :: SchemumExpression schema_alias view_alias db ('View view)
  -- ^ user defined view
  -> TypeExpression db (nullity ('PGcomposite view))
typeview = UnsafeTypeExpression . renderSchemumExpression
...
instance (Has schema_alias db schema, Has schemum_alias schema schemum)
  => IsQualified schema_alias schemum_alias
    (SchemumExpression db_alias schemum_alias db schemum) where
